...
from traj_complete_ros.measures import avg_acc
...
# @numba.jit
def calc_dtw(ref_curve, exec_data, radius=1000):
    ...
    distance, path = fastdtw(ref_curve, exec_data, dist=euclidean, radius=radius)
...
if __name__ == "__main__":
    ...
    path_to_experiment_table = os.path.expanduser("~/traj_complete_log/experiments_v_vs_error_match_real.xls")
    ...
    for idx, experiment in experiments.iterrows():
        ...
        if ("end" in str(experiment["name"]).lower()):
            print('Reached end of experiment table.')
            break
        ...
        try:
            if "skip" in str(experiment["skip_eval"]).lower():
                ...
                continue
            if "done" in str(experiment["skip_eval"]).lower():
                print('Experiment evaluation skipped. Already done.')
                continue
        except ValueError:
            pass
        ...
        log_ana = LogAnalyzer(urdf_path, world_frame='world', eef='tool0')
        ...
        _, axs = plot_plan(log_ana.get_log_as_plan(extra_trim=[1,1]), 'Joint Motion and End-Effector Speed', save=result_folder + "/Joint_motion_eef_vel_{}_{}_{}_{}.png".format(experiment['name'],experiment['default_pattern'],vel_val, experiment['executor_engine']), show=False, ret=True)
        ...
        experiment['M_A_norm'] = avg_acc(log_ana.get_acc(extra_trim=[1,1]), w=1/log_ana.get_acc_limit()[:,1])


        print('hallo')

        experiment['skip_eval'] = 'done'
