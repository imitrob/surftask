...
from traject_msgs.msg import ContourArrayStamped, LoggingTask
import actionlib
...
class UI(object):
    ...
    def __init__(self, goal=None, log_name=None):

        rospy.init_node('just_do_it_right_now')
        # self.config = config

        self.result = -1  # didn't run
        ...
        rospy.sleep(0.5)
        if goal is not None:
            self.sendGoal(goal=goal, log_name=log_name)

    def get_state(self):
        return self.result

    def sendGoal(self, goal, log_name):
