...
class CurveExecutor(object):
    def __init__(self):
        ...
        self.mg = MoveGroupCommander('panda_arm')
        self.traj_client = TrajectoryActionClient(arm='r2',
                                                  moveGroupCommander=self.mg,
                                                  topic="/position_joint_trajectory_controller/follow_joint_trajectory",
                                                  robotCommander=None)

        self.arm = ArmInterface()

        # self.arm.move_to_neutral()
        self.arm.move_to_joint_positions(positions=self.arm._neutral_pose_joints, use_moveit=False)
        ...
        self.js_sub = rospy.Subscriber('/joint_states', JointState, callback=self.js_cb, queue_size=1)
    ...
    def retime(self, plan):
        ...
        alims = len(lower)* [1.0]
        # panda limits: https://frankaemika.github.io/docs/control_parameters.html#controller-requirements
        alims = [15, 7.5, 10.0, 12.5, 15.0, 20.0, 20.0]

        ss = [pt.time_from_start.to_sec()*10.0 for pt in plan.joint_trajectory.points]
        ...
        # pc_cart_vel = ta.constraint.SecondOrderConstraint()

        instance = ta.algorithm.TOPPRA([], path)
